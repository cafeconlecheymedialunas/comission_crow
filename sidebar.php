    ...
    if (is_archive() || is_single()) :
        ...
        if ($recentposts_query->have_posts()) :
            ...
            while ($recentposts_query->have_posts()) :
                ...
                $output .= '<h4><a href="' . esc_url(get_the_permalink()) . '" title="' . sprintf(esc_attr__('Permalink to %s', 'comission_crow'), the_title_attribute([ 'echo' => false ])) . '" rel="bookmark">' . esc_html(get_the_title()) . '</a></h4>';
                $output .= '</li>';
            endwhile;
        endif;
        wp_reset_postdata();
