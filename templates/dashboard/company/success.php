...
$redirect_confirm = isset($_GET['redirect_confirm']) ? $_GET['redirect_confirm'] : '';

if ($session_id && $redirect_confirm === 'true'):
    try {
        // Obtener el ID de pago asociado con la sesión
        ...
        // Verificar si el proceso ya fue completado
        $payment_status = get_post_meta($payment_id, '_status');

       
        if ($payment_status[0] === 'payment_completed') {
            // Si el proceso ya fue completado, no hacer nada
            echo '<div class="alert alert-info">The payment has already been processed.</div>';
            return;
        }
        ...
        // Generar o recuperar la factura
        ...
        // Obtener y validar el post del pago
        $payment_post = get_post($payment_id);
        if (!$payment_post) {
            throw new Exception('Payment post not found.');
        }

        // Actualizar el estado del pago y el flag de completado
        ...
        $status_commission_request_history = Helper::add_item_to_status_history($commission_request_id, "");
        carbon_set_post_meta($commission_request_id, 'status_history', $status_commission_request_history);
