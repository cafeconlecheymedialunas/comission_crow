<?php
// Obtener términos de las taxonomías
...
// Obtener el post de la empresa asociada al usuario
...
// Obtener términos asociados al post de la oportunidad (si existe)
...
// Obtener el término de location actual
$current_country = '';
$current_city = '';
if (!empty($location)) {
    $location_term = get_term($location[0], 'location');
    if ($location_term && !is_wp_error($location_term)) {
        list($current_country, $current_city) = explode(' - ', $location_term->name, 2);
    }
}
...
<input type="hidden" id="current_country" value="<?php echo  $current_country; ?>"/>
		<input type="hidden" id="current_city" value="<?php echo  $current_city; ?>"/>
    <div class="col-md-6">
    ...
    <select id="country" name="country" data-location="<?php echo  $current_country; ?>" class="select2 form-select">
...
<div class="col-md-6">
    ...
    <select id="city" data-location="<?php echo  $current_city; ?>" name="city" class="select2 form-select" disabled>
...
<div class="col-md-12">
 ...
 <script src="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.13/js/select2.min.js"></script>
  ...
  jQuery(document).ready(function($) {
    ...
    // Obtener el país actual desde el campo oculto
    const currentCountry = $("#current_country").val();
    const currentCity = $("#current_city").val();
    ...
    $.ajax(settings).done(function(response) {
        ...
        // Limpiar y cargar países en el select
        $("#country").empty().append(`<option value="">Seleccione un país</option>`);
        ...
        if (currentCountry) {
            $("#country").val(currentCountry).trigger("change");
        }
    ...
    $("#country").change(function() {
        let countryName = $(this).val(); // Esto es el nombre del país
        ...
        if (countryName) {
            ...
            $.ajax(settings).done(function(response) {
                ...
                // Limpiar y cargar ciudades en el select
                $("#city").empty().append(`<option value="">Seleccione una ciudad</option>`);
                $.each(response.data, function(index, city) {
                    $("#city").append(`<option value="${city}">${city}</option>`);
                });

                // Seleccionar la ciudad actual si existe y coincide con el país
                if (currentCity && countryName === currentCountry) {
    ...
    // Si hay un país actual, cargar las ciudades automáticamente
    if (currentCountry) {
