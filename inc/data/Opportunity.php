...
public function save_opportunity()
{
    ...
    if (empty($data['country']) || empty($data['city'])) {
        $errors["location"][] = __("Country and City fields are required.");
    }

    // Verificar si el "location" ya existe como término
    $location_id = null;
    if (!empty($data['country']) && !empty($data['city'])) {
        $location_slug = sanitize_title($data['country'] . ' ' . $data['city']);

        // Verificar si ya existe el término "location"
        $existing_location = get_term_by('slug', $location_slug, 'location');

        if (!$existing_location) {
            // Si no existe, crear el nuevo término de ubicación (location)
            $new_location = wp_insert_term(
                $data['country'] . ' - ' . $data['city'], // Nombre del término
                'location', // Taxonomía
                [
                    'slug' => $location_slug,
                ]
            ...
            if (is_wp_error($new_location)) {
                $errors['location'][] = __("Error creating location.");
            } else {
    ...
    // Verificar si el usuario tiene asociado una empresa
    ...
    // Verificar si la oportunidad existe para actualizar
    ...
    // Si hay errores generales, devolverlos
    if (!empty($general_errors)) {
        wp_send_json_error(['general' => $general_errors]);
    }

    // Subir imágenes y materiales de soporte
    ...
    // Crear o actualizar la oportunidad
    ...
    // Asignar términos a la oportunidad
    ...
    if (!empty($location_id)) {
        wp_set_post_terms($opportunity_id, [$location_id], 'location');
    }

    wp_send_json_success(['message' => __('Opportunity created successfully'), 'opportunity_id' => $opportunity_id]);
}
